...
contract FundswapV2Pair is IFundswapV2Pair, FundswapV2ERC20{
    ...
    function _safeTransfer(address token, address to, uint value) private{
        ...
        require( success && (data.length == 0 || abi.decode(data, (bool))), 'FundswapV2: TRANSFER_FAILED');
    }
   ...
   // called once by the factory at time of deployment
    function initialize(address _token0, address _token1) external{
        require(msg.sender == factory, 'FundswapV2: FORBIDDEN'); // sufficient check
         token0 = _token0;
         token1 = _token1;  
    }
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'FundswapV2: INSUFFICIENT_LIQUIDITY_BURNED');
   ...
   // this low level funcion cal shoud be called from a contract that implement saftey checks
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if(data.length > 0) IFundswapV2Callee(to).fundswapV2Call(msg.sender, amount0Out, amount1Out, data); 
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        } 
        ...
        require(amount0In > 0 || amount1In > 0, 'FundswapV2: INSUFFICIENT_INPUT_AMOUNT'); 
        {
